...
.DESCRIPTION
    流程：
        ...
        5. 在 host 上掃描 offline 的 disk，取「排序在最後面」的那一顆，
           準備給 SUT 做 SCSI passthrough。
...
function Get-LinkSpeedLabel {
    ...
    if ($LinkSpeed -match '^\s*(?<num>\d+(?:\.\d+)?)\s*(?<unit>[GMK])bps') {
        ...
        switch ($Matches.unit) {
            'G' { return ('{0:0}G' -f $num) }
            'M' {
                if ($num -ge 1000) { return ('{0:0}G' -f ($num / 1000)) }
...
function Resolve-VmRoot {
    ...
    # 3) 沒有 D: → 列出所有「非系統 / 非開機」磁碟 (不分 online / offline)，讓使用者挑一顆來格式化成 D:
    Write-Host 'D: drive not found. Looking for non-system disks to provision as D:...' -ForegroundColor Yellow

    # 候選條件：不是系統 / 開機磁碟即可 (online、offline、RAW、已分割 都列出)
    $candidates = @(
        Get-Disk |
            Where-Object { $_.IsBoot -ne $true -and $_.IsSystem -ne $true } |
            Sort-Object Number
    )
    if ($candidates.Count -eq 0) {
        throw 'No non-system disk found on this host. Please add a disk and re-run, or pass -VmRoot explicitly.'
    }

    Write-Host 'Available non-system disks:' -ForegroundColor Green
...
# ============================================================================
# Step 4: 選擇 VM 規格 profile (Default / Max)
# ============================================================================

function Get-VmProfile {
    param([Parameter(Mandatory)][int]$RhelMajor)

    Write-Section 'Step 4 - Select VM profile (CPU / Memory)'
    ...
    $choice = Read-Choice -Prompt 'Select profile' -Valid $valid -Default 'D'

    $picked = if ($choice -eq 'M' -and $max) { $max } else { $default }
    Write-Host ("Selected profile: {0} vCPU / {1} GB" -f $picked.Cpu, $picked.MemGB) -ForegroundColor Cyan
    return $picked
}

# ============================================================================
# Step 5: 找出 host 上「排在最後面」的 offline disk (給 SUT 做 passthrough)
# ============================================================================

function Find-LastOfflineDisk {
    Write-Section 'Step 5 - Locate last offline disk for SUT passthrough'

    $offline = @(Get-Disk | Where-Object IsOffline -EQ $true | Sort-Object Number)
    ...
    $offline |
        Select-Object Number, FriendlyName,
                      ...
                      PartitionStyle, OperationalStatus, BusType |
        Format-Table -AutoSize | Out-String | Write-Host

    $last = $offline | Select-Object -Last 1
    Write-Host ("Picked last offline disk: Disk {0}  ({1}, {2:N1} GB)" -f `
                $last.Number, $last.FriendlyName, ($last.Size / 1GB)) -ForegroundColor Cyan
    return $last
}
...
try {
    ...
    # 把 Hyper-V host 預設的 VM config / VHD 路徑都指到 $vmRootDir (不要落在 C:)
    try {
    ...
    $vmProfile = Get-VmProfile -RhelMajor $iso.Major
    $offDisk   = Find-LastOfflineDisk
    ...
    $summary = [ordered]@{
        ...
        VmRoot             = $vmRootDir
        ProcessorCount     = $vmProfile.Cpu
        MemoryGB           = $vmProfile.MemGB
        VhdSizeGB          = $VhdSizeGB
